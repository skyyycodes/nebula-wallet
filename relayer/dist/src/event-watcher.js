...
export class EventWatcher {
    ...
    async startWatching(onApproval) {
    ...
    isApprovalEvent(event) {
        ...
        try {
            ...
            const topic0 = StellarSdk.xdr.ScVal.fromXDR(event.topic[0], 'base64');
    ...
    parseApprovalEvent(event) {
        try {
            ...
            let stellarAddress = '';
            if (event.topic && event.topic.length > 1) {
                const topic1 = StellarSdk.xdr.ScVal.fromXDR(event.topic[1], 'base64');
