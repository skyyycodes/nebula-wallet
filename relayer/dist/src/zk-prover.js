 ...
 */
function parseSphincsSignature(signature) {
    let offset = 0;
 ...
 */
function computeForsIndices(R, pkSeed, messageHash) {
 ...
 */
function computeHtSigCommitment(forsPk, pkRoot, htSig) {
 ...
 */
function computeForsPkFromSignature(forsSecrets, forsAuthPaths, leafIndices, pkSeed) {
 ...
 */
function prepareCircuitInputs(messageHash, publicKey, signature) {
    // Parse public key
    ...
    return {
        ...
        forsAuthPaths: forsAuthPaths.map(path => path.map(node => Array.from(node.map(b => b[0])))),
        leafIndices,
 ...
 */
function serializeProofToBytes(proof) {
 ...
 */
function serializePublicInputsToBytes(publicSignals) {
    const buffers = [];
 ...
 */
export async function generateZKProof(messageHash, publicKey, signature) {
    ...
    const { proof, publicSignals } = await snarkjs.groth16.fullProve(circuitInputs, WASM_FILE, ZKEY_FILE);
    const elapsed = (Date.now() - startTime) / 1000;
    console.log(`Proof generated in ${elapsed.toFixed(2)} seconds`);
    // Serialize for contract
    const proofBytes = serializeProofToBytes(proof);
 ...
 */
export async function verifyZKProofLocally(proof, publicSignals) {
