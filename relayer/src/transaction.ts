...
interface PendingApproval {
  ...
  expiresAt: number;
  consumed: boolean;
  sphincsSignature?: string; // Hex-encoded SPHINCS+ signature
}
...
export class TransactionHandler {
  ...
  async processApproval(txHash: string, stellarAddress: string): Promise<string> {
    ...
    const sphincsPublicKey = await getRegisteredPublicKey(
      CONFIG.CONTRACT_ID,
      approval.stellarAddress,
      this.sorobanServer
    );
    ...
    if (approval.sphincsSignature) {
      ...
      const isValid = verifySphincsSignature(
        signatureBytes,
        txHashBytes,
        sphincsPublicKey
      );
    ...
    // 7. Check relayer is authorized signer
    const relayerSigner = sourceAccount.signers.find(
      (s: StellarSdk.Horizon.HorizonApi.AccountSigner) => s.key === this.relayerKeypair.publicKey()
    );
  ...
  private async fetchPendingApproval(txHash: string): Promise<PendingApproval | null> {
    try {
  ...
  private parsePendingApproval(scVal: StellarSdk.xdr.ScVal): PendingApproval | null {
    try {
  ...
  private async markConsumed(txHash: string): Promise<void> {
    try {
      const contract = new StellarSdk.Contract(CONFIG.CONTRACT_ID);
