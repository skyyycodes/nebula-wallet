  ...
  try {
    ...
    const tx = new StellarSdk.TransactionBuilder(relayerAccount, {
      ...
      .addOperation(contract.call(
        ...
        StellarSdk.xdr.ScVal.scvBytes(Buffer.from(txHash, 'hex') as any),
        StellarSdk.xdr.ScVal.scvBytes(Buffer.from(txXdr, 'base64') as any),
        StellarSdk.xdr.ScVal.scvBytes(signatureBytes as any)
      ))
      .setTimeout(30)
      .build();
    ...
    try {
      ...
      const sphincsPublicKey = (parsedTx.memo as any).value as Uint8Array;
      ...
      const isValid = verifySphincsSignatureSimplified(
        signatureBytes,
      ...
      if (processingError && typeof processingError === 'object') {
        if ('response' in processingError) {
          const err = processingError as any;
...
app.post('/api/verify-and-submit', async (req, res) => {
  try {
    ...
    for (const op of parsedTx.operations) {
      ...
      const opSource = (op as any).source || undefined;
  ...
  try {
    ...
    const tx = new StellarSdk.TransactionBuilder(relayerAccount, {
      ...
      .addOperation(contract.call(
        ...
        StellarSdk.xdr.ScVal.scvBytes(Buffer.from(txHash, 'hex') as any),
        StellarSdk.xdr.ScVal.scvBytes(Buffer.from(txXdr, 'base64') as any),
        StellarSdk.xdr.ScVal.scvBytes(proofBytes as any),
        StellarSdk.xdr.ScVal.scvBytes(publicInputsBytes as any)
      ))
    ...
    if (getResponse.status === 'SUCCESS') {
      ...
      const preimageResult = (preimageSimulation as any).result?.retval;
      ...
      for (const op of parsedTx.operations) {
        if (op.type === 'payment') {
          ...
          newTxBuilder.addOperation(StellarSdk.Operation.pathPaymentStrictSend({
            sendAsset: (op as any).sendAsset,
            sendAmount: (op as any).sendAmount,
            destination: (op as any).destination,
            destAsset: (op as any).destAsset,
            destMin: (op as any).destMin,
            path: (op as any).path || []
          }));
        } else if (op.type === 'changeTrust') {
          newTxBuilder.addOperation(StellarSdk.Operation.changeTrust({
            asset: (op as any).line,
            limit: (op as any).limit
          }));
        }
