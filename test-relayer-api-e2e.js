...
/**
 * End-to-End Test: Complete Wallet Flow with Relayer API
 * Tests: wallet creation → funding → registration → locking → relayer API submission → transaction verification
 */
...
async function runE2ETest() {
  ...
  try {
    ...
    // STEP 7: Submit approval via relayer API
    console.log('✅ STEP 7: Submitting approval via relayer API...');
    ...
    console.log('   🔗 Stellar Expert:', `https://stellar.expert/explorer/testnet/tx/${apiResult.txHash}`);
    console.log('');

    // STEP 8: Wait for relayer to process and submit the actual transaction
    console.log('✅ STEP 8: Waiting for relayer to verify and submit transaction...');
    console.log('   (Relayer will verify SPHINCS+ signature off-chain)');
    console.log('   (Then submit the payment transaction to Stellar)');
    console.log('');
    
    const recipient = 'GBPXX4VUMWEU5MMKDK7PRBVH32AE2JODAAXE5VU2EIVXYHOKQDOLJ2DL'; // From payment tx
    let finalTxHash = null;
    
    // Poll for transaction on Horizon
    for (let i = 0; i < 60; i++) {
      await sleep(2000);
      process.stdout.write(`\r   Polling... ${i * 2}s / 120s`);
      
      try {
        // Check account operations to see if payment went through
        const operations = await horizon.operations()
          .forAccount(stellarKeypair.publicKey())
          .order('desc')
          .limit(10)
          .call();
        ...
        const recentPayment = operations.records.find(op => 
          op.type === 'payment' && 
          op.from === stellarKeypair.publicKey() && 
          op.to === recipient &&
          op.amount === '10'
        );
        ...
        if (recentPayment) {
          ...
          console.log('\n   ✓ Payment transaction found on-chain!');
          break;
        }
      } catch (error) {
        // Continue polling
    ...
    if (!finalTxHash) {
      console.log('\n');
      console.log('   ⚠️  Transaction not yet confirmed (may still be processing)');
      console.log('   Check the relayer logs for verification status:');
      console.log('   tail -f /tmp/relayer.log');
      console.log('');
      console.log('   Manual verification:');
      console.log('   - Approval TX:', apiResult.txHash);
      console.log('   - Check relayer is running: curl http://localhost:3001/api/health');
      return;
    }
    console.log('');

    // STEP 9: Verify transaction details
    console.log('✅ STEP 9: Verifying transaction on Stellar network...');
    const finalTx = await horizon.transactions()
      .transaction(finalTxHash)
      .call();
    
    console.log('   Transaction Hash:', finalTxHash);
    console.log('   Status:', finalTx.successful ? '✅ SUCCESS' : '❌ FAILED');
    console.log('   Source Account:', finalTx.source_account);
    console.log('   Operations:', finalTx.operation_count);
    console.log('   Fee:', finalTx.fee_charged, 'stroops');
    console.log('   🔗 Stellar Expert:', `https://stellar.expert/explorer/testnet/tx/${finalTxHash}`);
    console.log('');
    ...
    console.log(`  ✅ SPHINCS+ signature verified off-chain by relayer`);
    console.log(`  ✅ Transaction confirmed on Stellar network`);
    console.log(`  ✅ 10 XLM sent from locked wallet to ${recipient}`);
    console.log('');
    console.log('🔗 View on Stellar Explorer:');
    console.log(`   Lock TX:     https://stellar.expert/explorer/testnet/tx/${lockResponse.id}`);
    ...
    console.log('');
    console.log('🎉 Quantum-safe wallet with post-quantum signatures working end-to-end!');
    console.log('');
